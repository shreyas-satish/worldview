(function() {
  ...
  WorldView = (function() {
    ...
    function WorldView(mapconfig) {
      ...
      this.drawFeature = __bind(this.drawFeature, this);
    ...
    WorldView.prototype.initToolbar = function(options) {
      this.initToolbarControls(options);
      return this.initToolbarDOM();
    };

    WorldView.prototype.initToolbarControls = function(options) {
      var key, _results;
      this.controls = {
        "point": this.drawFeature(options.vectorLayer, OpenLayers.Handler.Point, options.callback),
        "line": this.drawFeature(options.vectorLayer, OpenLayers.Handler.Path),
        "polygon": this.drawFeature(options.vectorLayer, OpenLayers.Handler.Polygon, options.callback),
        "drag": new OpenLayers.Control.DragFeature(options.vectorLayer)
      };
      _results = [];
      for (key in this.controls) {
        _results.push(this.map.addControl(this.controls[key]));
      }
      return _results;
    };

    WorldView.prototype.initToolbarDOM = function() {
      var div, mapDom;
      div = OpenLayers.Util.createDiv(this.map.id, null, null, null, "absolute", null, null, null);
    ...
    WorldView.prototype.drawFeature = function(vectorLayer, handler, callback) {
      if (callback == null) {
        callback = function() {
    ...
    WorldView.prototype.toggleControl = function(element) {
      var control, key, _results;
      _results = [];
      for (key in this.controls) {
        control = this.controls[key];
        if ((element.value === key) || (element.title === key)) {
  ...
  WorldView.VectorLayer = (function() {
    ...
    VectorLayer.prototype.addPolygon = function(points) {
      var feature, linear_ring;
    ...
    VectorLayer.prototype.addLine = function(points) {
      var linefeature;
      linefeature = new OpenLayers.Feature.Vector(new OpenLayers.Geometry.LineString(points), null, {});
      this.addFeature(feature);
      return feature;
    };
    ...
    VectorLayer.prototype.addCircle = function(circleOptions) {
      var circle, feature;
      circle = OpenLayers.Geometry.Polygon.createRegularPolygon(circleOptions.point, circleOptions.radius, 20);
